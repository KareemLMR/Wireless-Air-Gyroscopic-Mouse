...
void* listen_for_unicast(void* receiver)
{
    ...
    while (1)
    {
        // Receive packet
        pthread_mutex_lock(&receiver_thread->server_mutex);
        if ((receiver_thread->unicast_socket = accept(receiver_thread->sockfd, (struct sockaddr *)&receiver_thread->servaddr,
...
void wait_for_message(receiver_t* receiver, poller_t* poller)
{
    ...
    printf("ENtered!\n");
    poller->flag = false;
    mtx_init(&poller->mutex, mtx_plain);
    cnd_init(&poller->cond);
    ...
    while (!poller->flag)
    {
        cnd_wait(&poller->cond, &poller->mutex);
    }
